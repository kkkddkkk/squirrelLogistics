...
public interface DeliveryCargoRepository extends JpaRepository<DeliveryCargo, Long> {
	...
	@Query("SELECT r.distance, r.totalCargoWeight, r.estimatedFee, "
			+ "COUNT(dw.dropOrder), ct.extraFee, ct.handlingId " + "FROM DeliveryCargo dc " + "JOIN dc.cargoType ct "
			+ "JOIN dc.deliveryWaypoint dw " + "JOIN dw.deliveryRequest r " + "WHERE r.requestId = :requestId "
