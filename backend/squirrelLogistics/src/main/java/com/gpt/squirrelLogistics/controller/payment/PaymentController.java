...
public class PaymentController {
	...
	public Map<String, Long> tryFirstPayment(@PathVariable(name = "paymentId") Long paymentId,
	...
	public Map<String, Long> trySecondPayment(@PathVariable(name = "paymentId") Long paymentId,
			@RequestBody PaymentDTO paymentDTO) {
		paymentDTO.setPaymentId(paymentId);
	...
	public Map<String, Long> successFirstPayment(@PathVariable(name = "paymentId") Long paymentId,
			@RequestBody PaymentSuccessDTO paymentSuccessDTO) {
	...
	public Map<String, Long> successSecondPayment(@PathVariable(name = "paymentId") Long paymentId,
			@RequestBody PaymentSuccessDTO paymentSuccessDTO) {
		paymentSuccessDTO.setPaymentId(paymentId);
	...
	public Map<String, Long> successRefundPayment(@PathVariable(name = "paymentId") Long paymentId,
	...
	public Map<String, Long> failureSecondPayment(@PathVariable(name = "paymentId") Long paymentId,
			@RequestBody PaymentFailureDTO paymentFailureDTO) {
	...
	// 작성자: 고은설.
	// 기능: 추가 결제 없이 2차결제 완료 처리.
	@PutMapping("/second/{paymentId}/complete-without-payment")
	public ResponseEntity<?> completeWithoutPayment(@PathVariable(name = "paymentId") Long paymentId) {
		try {
			Long result = paymentService.completeWithoutPayment(paymentId);
			return ResponseEntity.ok(result);
		} catch (IllegalArgumentException e) {
			return ResponseEntity.status(404).body("PAYPENT_NOT_FOUND");
		}catch (Exception e) {
			log.info("error={}",e.getMessage());
			return ResponseEntity.status(404).body("UNKNOWN_ERROR");
		}
