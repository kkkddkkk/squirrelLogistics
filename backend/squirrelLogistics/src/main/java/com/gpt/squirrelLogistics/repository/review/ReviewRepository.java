...
//	List<Object[]> findReviewContent(@Param("reviewId") Long reviewId);
	...
	@Query("SELECT r FROM Review r " + "JOIN r.deliveryAssignment da " + "JOIN da.driver d "
			+ "WHERE d.driverId = :driverId")
	List<Review> findByDriverId(@Param("driverId") Long driverId);

	// companyId로 리뷰 조회
	@Query("SELECT r FROM Review r " + "JOIN r.deliveryAssignment da " + "JOIN da.deliveryRequest dr "
			+ "JOIN dr.company c " + "WHERE c.companyId = :companyId ")
	...
	@Query("""
			    ...
			    where d.driverId = :driverId
			        and r.stateEnum in (
			    com.gpt.squirrelLogistics.enums.review.StateEnum.SUBMITTED,
			    com.gpt.squirrelLogistics.enums.review.StateEnum.EDITED
			)
			    order by r.regDate desc
			    """)
	Page<DriverReviewCardResponseDTO> findAllSubmittedByDriverId(@Param("driverId") Long driverId, Pageable pageable);
